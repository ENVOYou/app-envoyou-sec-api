...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Obtain reCAPTCHA token and verify it with backend before attempting sign-in
      ...
      const token = await new Promise<string>((resolve, reject) => {
        grecaptchaAny.enterprise.ready(() => {
          grecaptchaAny.enterprise.execute(siteKey, { action: 'login' }).then((t: string) => resolve(t)).catch(reject)
        })
      })
